...
kmeans_mapped_tbl %>%
    ...
    labs(
        ...
        caption = "Conclusion: Based on the Skree Plot, we select 4 clusterse to segment the customer base"
    )
...
# Cluster 1 - High/Medium Price, Mountain Model Preference, Carbon Frame
cluster_trends_tbl %>%
    filter(.cluster == 1) %>%
...
# Cluster 2 - High/Medium Price, Road Model Preference, Carbon Frame
get_cluster_trends(cluster = 2)

# Cluster 3 - Low/Medium Price, Road Model Preference
get_cluster_trends(cluster = 3)

# Cluster 4 - Low/Medium Price, Mountain Model Preference, Aluminum Frame
